...
- [x] **Platform factory stubs**: re-home existing Linux backends behind `get_input_backend()` /
      `get_window_watcher()` / OS action defaults (behaviour unchanged on KDE)
- [x] QML: **bind focused app → profile**, list/edit/remove bindings, show current focus class
- [x] QML: **profile lifecycle** (create, rename, duplicate, delete; set default)
- [x] Wire action **search**; hide empty library categories
- [x] Dirty guards: warn on profile switch / quit if unsaved
...
- [x] First-run: **Accessibility** (and any serial) permission copy
...
- [x] Real search/filter; platform-neutral defaults (no hard-coded `konsole`/`dolphin` only)
- [x] Macros (`multi` sequence)
- [x] Encoder **adjustments** (relative steps: volume, scrub, …), see **D.1**
- [x] Per-control rotation tuning: invert, sensitivity, acceleration curve (**D.1**)
- [x] **Scroll / mouse-wheel** action type (`input_backend` has no scroll verb today) (**D.1**)
- [x] Move action execution off the device reader thread (**D.1** prerequisite)
...
- [x] Ship a **starter profile** (Media / Editing / Browser workspaces, labelled and
      coloured), generated by `scripts/make_starter_profiles.py` and opened on a first run
- [x] Ship a **ready-made application deck** (Visual Studio Code: five workspaces, 165 controls),
      generated by `scripts/make_vscode_profile.py`, as both a useful profile and a worked example
- [x] **docs/HOW-TO-BUILD.md**: the whole process end to end, for users and for an assistant
      building a profile on someone's behalf
- [x] Applications own their profiles, carry the match keys, and hold pages that switch
      profile on the window title (`app_paths`, `profile_manager`)
- [ ] Match on `bundle_id` as well as `wm_class` in the same rules (§4.4)
...
- [x] Automated tests (`tests/`, 812 checks): schema load/migrate, profile resolution,
...
- [x] Dynamic-mode full UX in QML (bind / list / remove / default)
- [x] Profile create / rename / duplicate / delete / import / export
- [x] Action library search + non-empty categories + OS-aware defaults
- [x] Brightness, reconnect, backend health in UI
- [x] `send_hotkey` key-delay fix: standalone latency win (§5.D.1)
- [x] Starter profile with real defaults, opened on a first run
- [x] Live / Live S mirror fidelity

Medium-term (Phase C):

- [x] Macro / multi-step actions
...
- [x] Wire action search; filter chips optional
- [x] Workspace indicator / name in header
- [x] Dirty-state safety on switch / quit
